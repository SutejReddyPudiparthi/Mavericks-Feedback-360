...
const classifySentiment = (text) => {
  if (!text || text.length < 10)
    return { label: "Insufficient Data", score: null };
  const lower = text.toLowerCase();
  let pos = 0,
    neg = 0;
  SENTIMENT_KEYWORDS.Positive.forEach((w) => {
    if (lower.includes(w)) pos++;
  });
  SENTIMENT_KEYWORDS.Negative.forEach((w) => {
    if (lower.includes(w)) neg++;
  });
  if (pos === 0 && neg === 0) return { label: "Neutral", score: 0.6 };
  if (pos >= neg)
    return { label: "Positive", score: +(pos / (pos + neg)).toFixed(2) };
...
export const runSentimentForCycle = async (cycleId) => {
  ...
  for (const form of forms) {
    ...
    for (const { name, text } of fields) {
      ...
      const existing = await SentimentResult.findOne({
        where: { formId: form.id, fieldName: name },
      });
      if (existing) {
...
export const runThemeClusteringForCycle = async (cycleId) => {
  ...
  const allText = forms.flatMap((f) =>
    [f.keyLearnings, f.suggestedImprovements].filter(Boolean),
  );
  const themes = {};
  ...
  await ThemeCluster.destroy({ where: { cycleId } });
  for (const [label, count] of topThemes) {
    const samples = allText
      .filter((t) => t.toLowerCase().includes(label))
      .slice(0, 3);
    await ThemeCluster.create({
      cycleId,
      themeLabel: label,
      frequencyCount: count,
      sampleResponses: JSON.stringify(samples),
    });
  }
};
...
export const getTrainerRecommendations = async (domain) => {
    ...
    include: [
      {
        ...
        include: [
          {
            model: "FeedbackCycle",
            include: [{ model: MaverickFeedbackForm }],
          },
        ],
      },
    ],
  ...
  const ranked = trainers
    .map((trainer) => {
      const allForms = (trainer.Sessions || []).flatMap((s) =>
        (s.FeedbackCycles || []).flatMap((c) =>
          (c.MaverickFeedbackForms || []).filter(
            (f) => f.status === "Submitted",
          ),
        ),
      );
      const sessionCount = trainer.Sessions?.length || 0;
      if (sessionCount < 2)
        return { trainer, compositeScore: null, sessionCount, avgRating: null };
