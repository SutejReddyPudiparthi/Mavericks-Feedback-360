...
import { User, SessionParticipant } from "../models/index.js";
...
router.get("/template", (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    "attachment; filename=participant_template.xlsx",
  );
  res.setHeader(
    "Content-Type",
...
router.post("/upload/:sessionId", upload.single("file"), async (req, res) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!row["Full Name"])
      rowErrors.push({ row: rowNum, field: "Full Name", message: "Required" });
    ...
    else if (!emailRegex.test(row["Email Address"]))
      rowErrors.push({
        row: rowNum,
        field: "Email Address",
        message: "Invalid format",
      });
    if (!row["Supervisor ID"])
      rowErrors.push({
        row: rowNum,
        field: "Supervisor ID",
        message: "Required",
  ...
  if (errors.length)
    return res.status(422).json({ imported: 0, skipped: rows.length, errors });
  ...
  for (const row of valid) {
    ...
    const supervisor = await User.findOne({
      where: { employeeId: row["Supervisor ID"], role: "Supervisor" },
    });

    const existing = await SessionParticipant.findOne({
      where: { sessionId: req.params.sessionId, userId: user.id },
    });
    if (existing) {
      skipped++;
      continue;
    ...
    await SessionParticipant.create({
      ...
      projectAssignmentDate: row["Project Assignment Date"]
        ? new Date(row["Project Assignment Date"])
        : null,
    });

    if (supervisor?.id && user.supervisorId !== supervisor.id) {
      await user.update({ supervisorId: supervisor.id });
    }
