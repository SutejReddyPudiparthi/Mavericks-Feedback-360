...
export function SessionsList() {
  ...
  useEffect(() => {
    api
      .get("/master/sessions?pageSize=50")
      .then((r) => setSessions(r.data.data));
    api.get("/master/courses?status=Active").then((r) => setCourses(r.data));
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    try {
      const { data } = await api.post("/master/sessions", {
        ...form,
...
export function SessionDetail() {
  ...
  useEffect(() => {
    api.get(`/master/sessions/${id}`).then((r) => {
      const sessionData = r.data;
      setSession(sessionData);
      const cycles = sessionData.cycles || sessionData.FeedbackCycles;
      const mCycle = cycles?.find((c) => c.cycleType === "Maverick");
      if (mCycle)
        api
          .get(`/feedback/cycles/${mCycle.id}`)
          .then((cr) => setCycleData(cr.data));
    });
  }, [id]);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      api.get(`/master/sessions/${id}`).then((r) => setSession(r.data));
    } catch (err) {
  ...
  const handleOverride = async () => {
    ...
    const cycles = session?.cycles || session?.FeedbackCycles;
    const cycle = cycles?.find((c) => c.cycleType === "Maverick");
    if (!cycle) return;
    try {
      await api.post(`/feedback/cycles/${cycle.id}/close-override`, {
  ...
  const handleDownloadTemplate = async () => {
    const res = await api.get("/participants/template", {
      responseType: "blob",
    });
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === "roster" && (
        <div className="card">
          <p className="text-sm text-gray-500 mb-4">
            {(session.participants || session.SessionParticipants || []).length}{" "}
            participants enrolled (capacity: {session.capacity})
          </p>
          <Table
            columns={[
              ...
              { key: "email", label: "Email", render: (r) => r.user?.email },
              {
                key: "enrolled",
                label: "Enrolled",
                render: (r) => new Date(r.enrolledAt).toLocaleDateString(),
              },
            ]}
            data={session.participants || session.SessionParticipants || []}
          />
        </div>
      )}
        ...
        <div className="card space-y-4">
          ...
          {cycleData ? (
            <>
              ...
              <CompletionBar
                pct={cycleData.completionPct}
                threshold={cycleData.threshold}
              />
            ...
            <p className="text-gray-400 text-sm">
              No feedback cycle found for this session.
            </p>
          )}
        </div>
      )}
      ...
      <Modal
        ...
        title="Close Cycle (Override)"
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            You are closing this cycle before the completion threshold is met.
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Explain why you are closing this cycle early..."
            />
            <p className="text-xs text-gray-400 mt-1">
          ...
          <div className="flex gap-3">
            <button onClick={handleOverride} className="btn-danger flex-1">
              Confirm Override
            </button>
            <button
